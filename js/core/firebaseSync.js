...
} from "./firebaseRuntime.js?v=20260912-portal-v2-home-reference-composition-live-cover-005-v1";
import { buildTournamentDeletionCallablePayload } from "./tournamentDeletionClient.js?v=20260912-portal-v2-home-reference-composition-live-cover-005-v1";
import {
  COMPETITION_TYPES,
  getCompetitionType,
  getCompetitionTypeFromTournamentType
} from "../data/competitionTypes.js?v=20260912-portal-v2-home-reference-composition-live-cover-005-v1";
import { makeAccessSession, normalizeRole, normalizeTournamentAccess } from "./roles.js?v=20260912-portal-v2-home-reference-composition-live-cover-005-v1";
import {
  USER_ACCESS_BOOTSTRAP_ERROR,
  buildUserAccessBootstrapPlan,
  diagnoseUserAccessBootstrap,
  readUserAccessBootstrapTournaments
} from "./userAccessBootstrap.js?v=20260912-portal-v2-home-reference-composition-live-cover-005-v1";
import { prepareHistoricalReconciliationDryRunRequest } from "./historicalReconciliationDryRun.js?v=20260912-portal-v2-home-reference-composition-live-cover-005-v1";
import { normalizeScoringButtonLayouts } from "../data/defaultScoringButtonLayouts.js?v=20260912-portal-v2-home-reference-composition-live-cover-005-v1";
import {
...
} from "../broadcast/broadcastRealtimeTransport.js?v=20260912-portal-v2-home-reference-composition-live-cover-005-v1";
import {
  buildPublicProjection,
  getPublicProjectionSignature,
  reconcilePublicProjection
} from "../public/publicProjection.js?v=20260912-portal-v2-home-reference-composition-live-cover-005-v1";
import {
  adaptPublicProjectionToLegacyLive
} from "../public/publicProjectionLegacyAdapter.js?v=20260912-portal-v2-home-reference-composition-live-cover-005-v1";
import {
  diagnosePublicProjectionFirebaseCompatibility,
  normalizePublicProjectionForFirebase,
  validatePublicProjection
} from "../public/publicProjectionSchema.js?v=20260912-portal-v2-home-reference-composition-live-cover-005-v1";
import {
...
} from "./publicProjectionOutbox.js?v=20260912-portal-v2-home-reference-composition-live-cover-005-v1";
import {
  normalizePendingScoreReview,
  validatePendingScoreReview
} from "./pendingScoreReview.js?v=20260912-portal-v2-home-reference-composition-live-cover-005-v1";
import {
...
} from "./timerRules.js?v=20260912-portal-v2-home-reference-composition-live-cover-005-v1";
import { buildOfficialCurrentTimerContext } from "./officialTimerOrchestration.js?v=20260912-portal-v2-home-reference-composition-live-cover-005-v1";
import {
  BRAKE_REVIEW_ACTIONS,
  applyBrakeReviewCommand,
  getBrakeReviewStateFromTimer,
  isBrakeReviewProfile
} from "./brakeReviewPhase.js?v=20260912-portal-v2-home-reference-composition-live-cover-005-v1";
...
export async function uploadFirebaseTournamentPublicAsset(request = {}) {
  const callable = httpsCallable(getFirebaseFunctions(), "uploadCharroProTournamentPublicAsset");
  try {
    const response = await callable(cloneFirebaseBroadcastValue(request));
    return { ok: response?.data?.ok === true, ...(cloneFirebaseBroadcastValue(response?.data || {})) };
  } catch (error) {
    return {
      ok: false,
      reason: String(error?.details?.reason || error?.code || error?.message || "public-branding-asset-upload-failed")
        .replace(/^functions\//, "")
        .slice(0, 160)
    };
  }
...
export async function deleteFirebaseTournament(tournamentId, actor = {}) {
  ...
  try {
    ...
    const authorityDetail = error?.details?.details || {};
    const backupDiagnostic = authorityDetail?.backup && typeof authorityDetail.backup === "object"
      ? {
          stage: String(authorityDetail.backup.failureStage || "").slice(0, 80),
          code: String(authorityDetail.backup.failureCode || authorityDetail.backup.reason || "").slice(0, 160),
          diagnosticId: String(authorityDetail.backup.diagnosticId || authorityDetail.backup.backupId || "").slice(0, 80)
        }
      : null;
  ...
  try {
    const transaction = await runTransaction(ref(getFirebaseDatabase(), path), (currentValue) => {
      ...
      const shouldFinish = request.action === BRAKE_REVIEW_ACTIONS.AUTHORIZE
        || request.action === BRAKE_REVIEW_ACTIONS.CONFIRM_DISQUALIFICATION
        || live.expired;
...
function normalizePublicTournamentState(tournamentState = {}) {
  ...
  return {
    ...
    officialScoreLedger: tournamentState.officialScoreLedger || {},
...
export async function runFirebaseHistoricalReconciliationDryRun(request = {}) {
  const prepared = prepareHistoricalReconciliationDryRunRequest(request);
  if (!prepared.ok) return { ok: false, code: "invalid-argument", reason: prepared.reason };

  try {
    const callable = httpsCallable(getFirebaseFunctions(), "reconcileCharroProHistoricalResults");
    const result = await callable(prepared.payload);
    return { ok: true, plan: cloneFirebaseBroadcastValue(result?.data || {}) };
  } catch (error) {
    return {
      ok: false,
      code: String(error?.code || "functions-error"),
      reason: String(error?.details?.reason || error?.message || error?.code || "functions-error")
    };
  }
}
...
function inflateTournamentStatePayload(tournamentId, record = {}) {
  ...
  const state = {
    ...
    officialScoreLedger: record.officialScoreLedger || {},

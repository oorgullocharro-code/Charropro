import { SUERTES, getTournamentSuertes } from "../data/suertes.js?v=20260622-prepare-gate1";
import { normalizeGraphicsConfig, readLocalGraphicsConfig } from "./graphicsConfig.js?v=20260622-prepare-gate1";
import { buildOfficialPackage } from "./officialFormat.js?v=20260622-prepare-gate1";
import { buildCharreadaLeaderboard, buildTournamentStandingColumns, buildTournamentTeamStandings, calculateAttemptTotal } from "./scoring.js?v=20260622-prepare-gate1";
import { getActiveCharreada, getActiveTournament, getCurrentContext, getTeam, getTournamentCharreadas, LIVE_TIMER_KEY, scoreKey, state } from "./state.js?v=20260622-prepare-gate1";
import { getLiveChannelFromUrl, getTournamentLiveChannel, isFirebaseLiveConfigured, publishFirebaseLive, publishFirebaseTurn } from "./firebaseSync.js?v=20260622-prepare-gate1";
import { getTimerScopeKey, getTimerView } from "./timerRules.js?v=20260622-prepare-gate1";

let syncTimer = null;
let firebaseSyncTimer = null;
const SYNC_OWNER_KEY = "charropro_sync_owner_v1";
...
export function claimGoogleSyncControl() {
  try {
    localStorage.setItem(SYNC_OWNER_KEY, JSON.stringify({
...
function hasGoogleSyncControl() {
  try {
    const raw = localStorage.getItem(SYNC_OWNER_KEY);
    if (!raw) {
...
function getSyncClientId() {
  try {
    const key = "charropro_sync_client_id";

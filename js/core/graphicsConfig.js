export const GRAPHICS_CONFIG_KEY = "charropro_graphics_config_v1";
...
export function readLocalGraphicsConfig() {
  try {
    const raw = localStorage.getItem(GRAPHICS_CONFIG_KEY);
...
export function writeLocalGraphicsConfig(config) {
  ...
  localStorage.setItem(GRAPHICS_CONFIG_KEY, JSON.stringify(normalized));
  return normalized;
}

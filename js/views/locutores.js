import { getTournamentSuertes } from "../data/suertes.js?v=20260830-supervisor-tournament-deletion-nan-serialization-recovery-001-v1";
import { escapeHTML, html, moneylessNumber, showToast } from "../core/dom.js?v=20260830-supervisor-tournament-deletion-nan-serialization-recovery-001-v1";
import { getCharroName } from "../core/sync.js?v=20260830-supervisor-tournament-deletion-nan-serialization-recovery-001-v1";
import {
...
import { loadState, state, subscribeToLiveUpdates } from "../core/state.js?v=20260830-supervisor-tournament-deletion-nan-serialization-recovery-001-v1";
import {
...
console.info("[locutores] tournamentId recibido:", liveChannel || "");
...
function render() {
    ...
    root.innerHTML = html`
      <main class="locutor-page">
        <div class="empty">
          ...
          <p>Falta tournamentId en la URL.</p>
        </div>
      </main>
    `;
    return;
  ...
  if (!remotePayload) {
    root.innerHTML = renderWaitingOfficialLocutor();
    return;
  }

  const payload = remotePayload;
  const charreada = payload.charreada;
...
function renderWaitingOfficialLocutor() {
  return html`
    <main class="locutor-page">
      <div class="empty">
        <h1>CharroPro Locutores</h1>
        <p>Esperando publicación oficial...</p>
      </div>
    </main>
  `;
}
...
function ensureFirebaseLive() {
  ...
  firebaseUnsubscribe = subscribeFirebaseLiveCurrent(liveChannel, (response) => {
    ...
    if (!payload || !hasUsefulLivePayload(payload)) {
      remotePayload = null;
      render();
      return;
    }
    console.info("[event-test] locutores conectado", {
      tournamentId: liveChannel,
      publishedScoreId: payload.published?.id || ""
    });
...
function getLocutorTeamTotal(payload, charreada, turn) {
  ...
  if (remotePayload) {
    return Number(payload.published?.teamTotal || payload.published?.totalEquipo || payload.teamTotal || payload.turn?.teamTotal || 0);
  }

import { escapeHTML, html, showToast } from "../core/dom.js?v=20260830-supervisor-tournament-deletion-nan-serialization-recovery-001-v1";
import { loadState, state } from "../core/state.js?v=20260830-supervisor-tournament-deletion-nan-serialization-recovery-001-v1";
import {
...
import { ROLES, getRoleLabel, hasTournamentAccess, isActiveAccessSession, roleCan } from "../core/roles.js?v=20260830-supervisor-tournament-deletion-nan-serialization-recovery-001-v1";
import { getTournamentIdFromUrl } from "../core/tournamentContext.js?v=20260830-supervisor-tournament-deletion-nan-serialization-recovery-001-v1";
...
root.addEventListener("click", (event) => {
  ...
  if (target.dataset.action === "open-judge-tournament") {
    if (!canOpenJudgeCharreada(target.dataset.id, target.dataset.charreada)) {
...
function renderGatewayBody() {
  ...
  console.info("[judge-context] tournamentId:", requestedId || "");
...
function renderSingleTournamentAccess(tournament) {
  return html`
    <section class="judge-shell judge-shell-access">
      <article class="card judge-register-card">
        <div class="card-header">
          <div>
...
function renderJudgeTournamentActions(tournament) {
  ...
  const activeCharreada = resolveJudgeActiveCharreada(tournament.id, charreadas);
  return html`
    <article class="judge-tournament-access-card">
      ...
      ${charreadas.length ? renderJudgeCharreadaGroups(tournament, charreadas, activeCharreada) : renderJudgeNoCharreadas(tournament)}
    </article>
  `;
}

function renderJudgeCharreadaGroups(tournament, charreadas, activeCharreada = {}) {
  const groups = [
    ["en_vivo", "En vivo"],
  ...
  return groups
    .map(([status, title]) => {
      const items = charreadas.filter((charreada) => normalizeCharreadaStatus(charreada.status, charreada.id, activeCharreada.id) === status);
      if (!items.length) return "";
      return html`
        <section class="judge-charreada-preview-group">
          <h3>${title}</h3>
          <div class="judge-charreada-preview-list">
            ${items.map((charreada) => renderJudgeCharreadaPreview(tournament, charreada, activeCharreada)).join("")}
          </div>
        </section>
      `;
    })
...
function renderJudgeCharreadaPreview(tournament, charreada, activeCharreada = {}) {
  const teamsCount = Array.isArray(charreada.teamIds) ? charreada.teamIds.length : 0;
  const isActive = activeCharreada.id
    ? activeCharreada.id === charreada.id
    : normalizeCharreadaStatus(charreada.status) === "en_vivo";
  const canScoreThisCharreada = roleCan(accessSession.role, "score") && isActive;
  const displayStatus = isActive ? "en_vivo" : normalizeCharreadaStatus(charreada.status, charreada.id, activeCharreada.id);
  return html`
    <article class="judge-charreada-preview ${isActive ? "active" : ""}">
      <div class="judge-charreada-main">
        <span>${escapeHTML(formatCharreadaStatus(displayStatus))}</span>
        <strong>${escapeHTML(charreada.name || "Charreada")}</strong>
        <dl class="judge-charreada-meta">
          <div>
            <dt>Fecha</dt>
            <dd>${escapeHTML(formatDateLabel(charreada.date) || "Sin fecha")}</dd>
          </div>
          <div>
            <dt>Hora</dt>
            <dd>${escapeHTML(formatTimeLabel(charreada.startTime) || "Sin hora")}</dd>
          </div>
          <div>
            <dt>Equipos</dt>
            <dd>${teamsCount}</dd>
      ...
      <div class="judge-tournament-actions">
        ${isActive ? html`
          <button class="button primary" data-action="open-judge-tournament" data-id="${escapeHTML(tournament.id)}" data-charreada="${escapeHTML(charreada.id)}" type="button" ${canScoreThisCharreada ? "" : "disabled"}>
            Calificar
          </button>
...
function canOpenJudgeCharreada(tournamentId, charreadaId) {
  if (!roleCan(accessSession.role, "score")) return false;
  const charreadas = getJudgeTournamentCharreadas(tournamentId);
  const activeCharreada = resolveJudgeActiveCharreada(tournamentId, charreadas);
  return Boolean(activeCharreada.id && activeCharreada.id === charreadaId);
}
...
function renderJudgeNoCharreadas(tournament) {
  return html`
    <div class="judge-empty">
      ...
      <p>Cuando el supervisor programe charreadas, apareceran aqui.</p>
    </div>
  `;
}
...
function buildJudgeHref(tournamentId, charreadaId = "") {
  ...
  if (charreadaId) {
    params.set("charreada", charreadaId);
    params.set("charreadaId", charreadaId);
  }
  console.info("[judge-context] activeCharreadaId resuelta:", charreadaId || "");
  console.info("[judge-context] fuente activeCharreadaId:", charreadaId ? getJudgeActiveCharreadaSource(tournamentId, charreadaId) : "none");
  console.info("[judge-context] entrando a charreada:", charreadaId || "");
  console.info("[event-test] juez entro a charreada", { tournamentId, charreadaId });
...
function normalizeCharreadaStatus(status, charreadaId = "", activeCharreadaId = "") {
  if (charreadaId && activeCharreadaId && String(charreadaId) === String(activeCharreadaId)) return "en_vivo";
  if (charreadaId && activeCharreadaId && String(status || "") === "en_vivo") return "programada";
  const value = String(status || "programada");
  if (value === "en_vivo" || value === "finalizada") return value;
  return "programada";
}

function resolveJudgeActiveCharreada(tournamentId, charreadas = []) {
  const activeId = getJudgeActiveCharreadaId(tournamentId);
  if (activeId) {
    const charreada = charreadas.find((item) => item.id === activeId);
    if (charreada) return { id: activeId, source: getJudgeActiveCharreadaSource(tournamentId, activeId) };
  }

  return { id: "", source: "none" };
}

function getJudgeActiveCharreadaId(tournamentId) {
  const remoteState = remoteTournamentStates.get(tournamentId) || {};
  const localTournament = (state.tournaments || []).find((tournament) => tournament.id === tournamentId) || {};
  return String(
    remoteState.activeCharreadaId ||
      ...
      localTournament.activeCharreadaId ||
      ""
  ).trim();
}

function getJudgeActiveCharreadaSource(tournamentId, charreadaId) {
  const remoteState = remoteTournamentStates.get(tournamentId) || {};
  const localTournament = (state.tournaments || []).find((tournament) => tournament.id === tournamentId) || {};
  if (charreadaId && (
    remoteState.activeCharreadaId === charreadaId ||
    remoteState.meta?.activeCharreadaId === charreadaId ||
    remoteState.info?.activeCharreadaId === charreadaId ||
    remoteState.tournamentState?.activeCharreadaId === charreadaId ||
    remoteState.tournament?.activeCharreadaId === charreadaId ||
    localTournament.activeCharreadaId === charreadaId
  )) return "tournament";
  return charreadaId ? "status" : "none";
}

import { SUERTES, TOURNAMENT_TYPES, getTournamentSuertes, getTournamentTypeConfig } from "./data/suertes.js?v=20260920-sabana-phase-compact-visual-parity-deploy-001-v1";
import { COMPETITION_TYPES, getCompetitionType, getCompetitionTypeFromTournamentType } from "./data/competitionTypes.js?v=20260920-sabana-phase-compact-visual-parity-deploy-001-v1";
import { CHARROPRO_APP_VERSION } from "./core/version.js?v=20260920-sabana-phase-compact-visual-parity-deploy-001-v1";
import {
  SCORING_BUTTON_GROUPS,
  normalizeScoringButtonGroup,
  normalizeScoringButtonLayouts
} from "./data/defaultScoringButtonLayouts.js?v=20260920-sabana-phase-compact-visual-parity-deploy-001-v1";
import {
  CALA_ADIC_SECTIONS,
  ...
  sumTeamPenalties
} from "./data/calaRules.js?v=20260920-sabana-phase-compact-visual-parity-deploy-001-v1";
import {
...
} from "./data/fmch2026PialesColeaderoRules.js?v=20260920-sabana-phase-compact-visual-parity-deploy-001-v1";
import {
...
} from "./data/fmch2026JineteosRules.js?v=20260920-sabana-phase-compact-visual-parity-deploy-001-v1";
import {
...
} from "./data/fmch2026TernaRules.js?v=20260920-sabana-phase-compact-visual-parity-deploy-001-v1";
import {
...
} from "./data/fmch2026ManganasPasoRules.js?v=20260920-sabana-phase-compact-visual-parity-deploy-001-v1";
import {
...
} from "./core/manganasFaenaScoring.js?v=20260920-sabana-phase-compact-visual-parity-deploy-001-v1";
import { closeModal, escapeHTML, html, moneylessNumber, showModal, showToast } from "./core/dom.js?v=20260920-sabana-phase-compact-visual-parity-deploy-001-v1";
import { applyLocalFmch2026RuleProfileDefault } from "./core/localRuleProfileDefaults.js?v=20260920-sabana-phase-compact-visual-parity-deploy-001-v1";
import {
  applyProductiveRuleProfilePolicy,
  resolveProductiveRuleProfileDefault
} from "./core/productiveRuleProfilePolicy.js?v=20260920-sabana-phase-compact-visual-parity-deploy-001-v1";
import { applyDefaultPublicPortalModules } from "./core/publicPortalModulePolicy.js?v=20260920-sabana-phase-compact-visual-parity-deploy-001-v1";
import { applyReleaseClassificationToNewTournament } from "./core/releasePolicy.js?v=20260920-sabana-phase-compact-visual-parity-deploy-001-v1";
import { getRuleProfile, getRuleProfileRulesByPhase } from "./data/ruleProfiles.js?v=20260920-sabana-phase-compact-visual-parity-deploy-001-v1";
import {
  SCORER_CONTEXT_STATUSES,
  isScorerContextReady,
  resolveScorerContextState
} from "./core/scorerContextResolution.js?v=20260920-sabana-phase-compact-visual-parity-deploy-001-v1";
import { EVENT_TYPES, buildEvent, registerEvent } from "./core/events.js?v=20260920-sabana-phase-compact-visual-parity-deploy-001-v1";
import { exportBackupJson, exportCurrentTournamentCsv } from "./core/exporters.js?v=20260920-sabana-phase-compact-visual-parity-deploy-001-v1";
import { downloadColeaderoXlsx, isColeaderoXlsxExport } from "./core/coleaderoXlsx.js?v=20260920-sabana-phase-compact-visual-parity-deploy-001-v1";
import { advanceAfterCompletedTernaSession, advanceScoringPointer, previousScoringPointer, resetScoringPointer } from "./core/flow.js?v=20260920-sabana-phase-compact-visual-parity-deploy-001-v1";
import { downloadOfficialFormatXlsx } from "./core/officialFormat.js?v=20260920-sabana-phase-compact-visual-parity-deploy-001-v1";
import {
...
} from "./core/ternaParticipantIdentity.js?v=20260920-sabana-phase-compact-visual-parity-deploy-001-v1";
import {
  getTeamLineupEntryValue,
  resolveTeamLineupEntries
} from "./core/teamLineup.js?v=20260920-sabana-phase-compact-visual-parity-deploy-001-v1";
import {
  ...
  getTimerScopeKey,
  getTimerView
} from "./core/timerRules.js?v=20260920-sabana-phase-compact-visual-parity-deploy-001-v1";
import {
  deriveOfficialTimerLiveDisplay,
  officialTimerDisplayStateLabel,
  officialTimerTicker,
  updateOfficialTimerDomDisplays
} from "./core/officialTimerLiveDisplay.js?v=20260920-sabana-phase-compact-visual-parity-deploy-001-v1";
import {
  TORO_TO_TERNA_HANDOFF,
  buildOfficialCurrentTimerContext,
  buildToroToTernaReadyDefinition,
  resolveOfficialCurrentTimerContext
} from "./core/officialTimerOrchestration.js?v=20260920-sabana-phase-compact-visual-parity-deploy-001-v1";
import {
  createScorerOfficialTimerConsumer,
  subscribeScorerOfficialTimerCurrent
} from "./core/scorerOfficialTimerConsumer.js?v=20260920-sabana-phase-compact-visual-parity-deploy-001-v1";
import {
...
} from "./core/brakeReviewPhase.js?v=20260920-sabana-phase-compact-visual-parity-deploy-001-v1";
import { buildStatisticalHistorySnapshot } from "./core/history.js?v=20260920-sabana-phase-compact-visual-parity-deploy-001-v1";
import { buildCharroProStatsCenter } from "./core/statistics.js?v=20260920-sabana-phase-compact-visual-parity-deploy-001-v1";
import {
  ...
  getTeamSuerteTotal,
  hasAttemptActivity
} from "./core/scoring.js?v=20260920-sabana-phase-compact-visual-parity-deploy-001-v1";
import { compareOfficialRankingRows } from "./core/officialRanking.js?v=20260920-sabana-phase-compact-visual-parity-deploy-001-v1";
import {
...
} from "./core/canonicalOfficialResults.js?v=20260920-sabana-phase-compact-visual-parity-deploy-001-v1";
import {
  adaptLegacyAttemptToV2,
  buildOfficialScoringAttemptSnapshot
} from "./core/scoringAttempt.js?v=20260920-sabana-phase-compact-visual-parity-deploy-001-v1";
import {
  buildScorerAttemptViewModel,
  buildScorerClassificationModel,
  buildScorerRuleButtonModel
} from "./core/scorerComponents.js?v=20260920-sabana-phase-compact-visual-parity-deploy-001-v1";
import {
  createScorerSaveLatencyTrace,
  summarizeScorerSaveLatency
} from "./core/scorerSaveLatency.js?v=20260920-sabana-phase-compact-visual-parity-deploy-001-v1";
import {
...
} from "./core/scorerInteractionLatency.js?v=20260920-sabana-phase-compact-visual-parity-deploy-001-v1";
import {
  ...
  sendToFirebaseTurn,
  sendToGoogleSheets
} from "./core/sync.js?v=20260920-sabana-phase-compact-visual-parity-deploy-001-v1";
import {
  ...
  runFirebaseHistoricalReconciliationDryRun,
...
} from "./core/firebaseSync.js?v=20260920-sabana-phase-compact-visual-parity-deploy-001-v1";
import {
...
} from "./core/tournamentPublicBranding.js?v=20260920-sabana-phase-compact-visual-parity-deploy-001-v1";
import {
  buildHistoricalReconciliationDryRunView,
  classifyHistoricalReconciliationDryRunResult
} from "./core/historicalReconciliationDryRun.js?v=20260920-sabana-phase-compact-visual-parity-deploy-001-v1";
import {
...
} from "./core/pendingScoreReview.js?v=20260920-sabana-phase-compact-visual-parity-deploy-001-v1";
import { ROLES, ROLE_OPTIONS, getRoleLabel, hasTournamentAccess, isActiveAccessSession, normalizeTournamentAccess, roleCan } from "./core/roles.js?v=20260920-sabana-phase-compact-visual-parity-deploy-001-v1";
import {
  buildTournamentUrl,
...
} from "./core/tournamentContext.js?v=20260920-sabana-phase-compact-visual-parity-deploy-001-v1";
import {
  buildPortalV2PublicPath,
  buildPortalV2PublicUrl
} from "./portalV2/portalV2Router.js?v=20260920-sabana-phase-compact-visual-parity-deploy-001-v1";
import {
...
} from "./core/supervisorNavigation.js?v=20260920-sabana-phase-compact-visual-parity-deploy-001-v1";
import { clearTournamentSandboxStorage } from "./core/localCache.js?v=20260920-sabana-phase-compact-visual-parity-deploy-001-v1";
import {
  createRoster,
  emptyAttempt,
  ...
  state,
  uid
} from "./core/state.js?v=20260920-sabana-phase-compact-visual-parity-deploy-001-v1";
...
const ACTION_CAPABILITIES = {
  ...
  "apply-jineteo-timing": "score",
  "adjust-mangana-floreo": "score",
  "toggle-mangana-floreo-detail": "score",
  "adjust-mangana-pulls": "score",
  "set-mangana-result": "score",
  "set-mangana-placed": "score",
  "set-paso-result": "score",
  "select-mangana-remate-shortcut": "score",
  "focus-mangana-remate-custom": "score",
  "save-mangana-remate": "score",
...
const PREPARATION_REQUIRED_ACTIONS = new Set([
  ...
  "set-mangana-placed",
  "set-paso-result",
  "select-mangana-remate-shortcut",
  "focus-mangana-remate-custom",
  "save-mangana-remate",
...
let historicalReconciliationDryRunState = {
  status: "READY",
  error: "",
  plan: null,
  classification: "",
  inputs: {
    ...
    teamId: "",
    sharedOpportunityId: "",
    reconciliationId: `dryrun-${Date.now().toString(36)}`
  }
};
...
function renderHistoricalReconciliationDryRunAdmin() {
  if (!isSupervisorAccess()) return "";
  const runtime = historicalReconciliationDryRunState;
  const view = runtime.plan ? buildHistoricalReconciliationDryRunView(runtime.plan) : null;
  const statusLabel = {
    READY: "Listo",
    VALIDATING: "Validando datos",
    DRY_RUN_RUNNING: "Consultando plan",
    DRY_RUN_SUCCESS: "Plan compatible",
    DRY_RUN_BLOCKED: "Plan bloqueado",
    AUTH_REQUIRED: "Sesión requerida",
    FORBIDDEN: "Solo Supervisor",
    SERVER_ERROR: "Error del servidor"
  }[runtime.status] || runtime.status;
  const readonlyBusy = ["VALIDATING", "DRY_RUN_RUNNING"].includes(runtime.status);
  return html`
    <article class="card" data-supervisor-historical-reconciliation-dry-run>
      <div class="card-header">
        <div>
          <h2 class="card-title">Diagnóstico de reconciliación histórica</h2>
          <p class="card-subtitle">DRY RUN — NO MODIFICA DATOS</p>
        </div>
        <span class="pill ${runtime.status === "DRY_RUN_SUCCESS" ? "green" : runtime.status === "DRY_RUN_BLOCKED" ? "red" : "blue"}">${escapeHTML(statusLabel)}</span>
      </div>
      <div class="card-body grid">
        <p class="card-subtitle">NO SE EJECUTARÁ RECONCILIACIÓN DESDE ESTA PANTALLA.</p>
        <div class="form-grid">
          <div class="wide"><label>Tournament ID</label><input id="historical-reconciliation-tournament-id" value="${escapeHTML(runtime.inputs.tournamentId || state.activeTournamentId || "")}" placeholder="torneo_..." ${readonlyBusy ? "disabled" : ""}></div>
          <div><label>Charreada ID</label><input id="historical-reconciliation-charreada-id" value="${escapeHTML(runtime.inputs.charreadaId)}" placeholder="charreada_..." ${readonlyBusy ? "disabled" : ""}></div>
          <div><label>Team ID</label><input id="historical-reconciliation-team-id" value="${escapeHTML(runtime.inputs.teamId)}" placeholder="equipo_..." ${readonlyBusy ? "disabled" : ""}></div>
          <div class="wide"><label>Shared Opportunity ID</label><input id="historical-reconciliation-shared-opportunity-id" value="${escapeHTML(runtime.inputs.sharedOpportunityId)}" placeholder="terna:torneo_...:op:4" ${readonlyBusy ? "disabled" : ""}></div>
          <div class="wide"><label>ID de consulta</label><input id="historical-reconciliation-id" value="${escapeHTML(runtime.inputs.reconciliationId)}" ${readonlyBusy ? "disabled" : ""}></div>
        </div>
        <button class="button primary" data-action="run-historical-reconciliation-dry-run" ${readonlyBusy ? "disabled" : ""}>Consultar DRY RUN</button>
        ${runtime.error ? html`<p class="pill red">${escapeHTML(runtime.error)}</p>` : ""}
        ${runtime.classification ? html`<p class="card-subtitle">Clasificación: ${escapeHTML(runtime.classification)}</p>` : ""}
        ${view ? renderHistoricalReconciliationDryRunResult(view) : ""}
      </div>
    </article>
  `;
}

function renderHistoricalReconciliationDryRunResult(view) {
  const rows = (items) => items.length
    ? html`<ul class="quick-team-list">${items.map((item) => html`<li>${escapeHTML(typeof item === "string" ? item : JSON.stringify(item))}</li>`).join("")}</ul>`
    : html`<span class="card-subtitle">NO DISPONIBLE EN RESPUESTA</span>`;
  const canonicalPR = Number.isFinite(view.canonicalPR) ? moneylessNumber(view.canonicalPR) : "NO DISPONIBLE EN RESPUESTA";
  const canonicalTotal = Number.isFinite(view.canonicalTotal) ? moneylessNumber(view.canonicalTotal) : "NO DISPONIBLE EN RESPUESTA";
  return html`
    <div class="grid">
      <div class="quick-team-list">
        <span class="pill">Modo ${escapeHTML(view.mode)}</span>
        <span class="pill">Cabeza canónica ${escapeHTML(view.currentRecordId || "NO DISPONIBLE EN RESPUESTA")}</span>
        <span class="pill">PR canónico ${escapeHTML(canonicalPR)}</span>
        <span class="pill">Total canónico ${escapeHTML(canonicalTotal)}</span>
        <span class="pill">Compatibilidad legacy ${view.legacyCompatibilityApplied ? "aplicada" : "no aplicada"}</span>
        <span class="pill">Evidencia de idempotencia ${view.idempotencyEvidencePresent ? "presente" : "no disponible"}</span>
      </div>
      <div><strong>Registros objetivo</strong>${rows(view.targetRecordIds)}</div>
      <div><strong>Registros que pasarían a histórico</strong>${rows(view.supersededRecordIds)}</div>
      <div><strong>Registros legacy compatibles</strong>${rows(view.legacyCompatibleRecordIds)}</div>
      <div><strong>Impacto público actual</strong>${rows(view.publicCurrent)}</div>
      <div><strong>Impacto público objetivo</strong>${rows(view.publicTarget)}</div>
      <div><strong>Alcance de escritura teórico</strong>${rows(view.writeScope)}</div>
      <p class="card-subtitle">Un respaldo validado sería obligatorio antes de cualquier operación futura. Esta pantalla no puede crear respaldo ni ejecutar el plan.</p>
    </div>
  `;
}

function readHistoricalReconciliationDryRunInputs() {
  const read = (id) => String(document.getElementById(id)?.value || "").trim();
  return {
    tournamentId: read("historical-reconciliation-tournament-id"),
    charreadaId: read("historical-reconciliation-charreada-id"),
    teamId: read("historical-reconciliation-team-id"),
    sharedOpportunityId: read("historical-reconciliation-shared-opportunity-id"),
    reconciliationId: read("historical-reconciliation-id")
  };
}

async function runHistoricalReconciliationDryRun() {
  if (!isSupervisorAccess()) return;
  const inputs = readHistoricalReconciliationDryRunInputs();
  historicalReconciliationDryRunState = {
    ...historicalReconciliationDryRunState,
    status: "VALIDATING",
    error: "",
    plan: null,
  ...
  render();
  historicalReconciliationDryRunState.status = "DRY_RUN_RUNNING";
  render();
  const result = await runFirebaseHistoricalReconciliationDryRun(inputs);
  const outcome = classifyHistoricalReconciliationDryRunResult(result);
  historicalReconciliationDryRunState = {
    ...historicalReconciliationDryRunState,
    status: outcome.state,
    classification: outcome.classification,
    error: result.ok ? "" : formatHistoricalReconciliationDryRunError(outcome.classification),
    plan: result.ok ? result.plan : null
  };
  render();
}

function formatHistoricalReconciliationDryRunError(classification = "") {
  const messages = {
    AUTH_REQUIRED: "Se requiere una sesión activa.",
    FORBIDDEN: "Solo un Supervisor autorizado puede consultar este plan.",
    INVALID_TOURNAMENT: "El torneo o su contexto no están disponibles.",
    SPORTING_MISMATCH: "BLOQUEADO: la evidencia deportiva no coincide.",
    IDENTITY_MISMATCH: "BLOQUEADO: la identidad deportiva no coincide.",
    REVISION_MISMATCH: "BLOQUEADO: la revisión cambió; vuelve a obtener el DRY RUN.",
    SUPERSESSION_MISMATCH: "BLOQUEADO: la sucesión histórica no coincide.",
    SUCCESSOR_MISSING: "BLOQUEADO: falta un registro sucesor.",
    ACTIVE_CONFLICT: "BLOQUEADO: existe un conflicto de cabezas activas.",
    UNKNOWN_INCOMPATIBILITY: "BLOQUEADO: incompatibilidad no reconocida por la autoridad.",
    SERVER_ERROR: "No se pudo consultar el plan de reconciliación."
  };
  return messages[classification] || messages.SERVER_ERROR;
}
...
function getPageHref(fileName, options = {}) {
  if (!options.sync) {
    ...
    if (fileName === "jueces.html" || fileName === "formato-federacion.html") addActiveCharreadaParam(params);
...
function formatDeleteTournamentError(result = {}) {
  ...
  const messages = {
    ...
    "tournament-delete-backup-create-failed": "No se pudo crear el respaldo obligatorio. El torneo no se elimino.",
    "tournament-delete-backup-validation-failed": "El respaldo se creo, pero no pudo validarse. El torneo no se elimino.",
  ...
  const message = messages[reason] || `No se pudo borrar el torneo. Motivo: ${reason || "error desconocido"}.`;
  const diagnosticId = String(result.detail?.backupDiagnostic?.diagnosticId || "").trim();
  return diagnosticId ? `${message} Codigo de diagnostico: ${diagnosticId}.` : message;
}
...
function applyPreparationSnapshot(snapshot = {}) {
  ...
  Object.values(snapshot.tournaments || {}).forEach((payload) => {
    ...
    replaceTournamentOfficialScoreLedger(tournamentId, remoteState.officialScoreLedger || {});
...
function replaceTournamentOfficialScoreLedger(tournamentId, nextLedger) {
  if (!state.officialScoreLedgers || typeof state.officialScoreLedgers !== "object") {
    state.officialScoreLedgers = {};
  }
  state.officialScoreLedgers[tournamentId] = nextLedger && typeof nextLedger === "object" ? nextLedger : {};
}
...
function clearPortalTournamentDetailCache(remoteIds = new Set()) {
  ...
  state.officialScoreLedgers = Object.fromEntries(
    Object.entries(state.officialScoreLedgers || {}).filter(([tournamentId]) => remoteIds.has(tournamentId))
  );
...
function removeLocalTournamentData(tournamentId) {
  ...
  if (state.officialScoreLedgers) delete state.officialScoreLedgers[tournamentId];
...
function renderSupervisorOverview() {
  ...
  return html`
    <section class="content">
      ...
      ${renderHistoricalReconciliationDryRunAdmin()}
    </section>
  `;
}
...
function compareResultsCompetitionRows(left, right) {
  return compareOfficialRankingRows(left, right);
}
    ...
    <div class="table-wrap">
      <table>
        <thead>
          <tr>
            <th>${escapeHTML(labels.nameHeader)}</th>
            ${suertes.map((suerte) => html`<th class="num">${escapeHTML(getCompactScoreSheetSuerteLabel(suerte))}</th>`).join("")}
            <th class="num">Total</th>
            <th class="num" title="Infracciones totales">Inf T</th>
          </tr>
        </thead>
        <tbody>
          ${teams.map((team) => {
            ...
            return html`
              <tr>
                ...
                ${suertes.map((suerte) => renderScoreSheetSuerteCell(canonicalTotals, suerte.id, { includeInfractions: false })).join("")}
        ...
        <tbody>
          ${rows
            .map(({ charreada, teamId, entry }) => {
                const canonicalTotals = getCanonicalResultsTeamTotals(charreada.id, teamId);
                return html`
                  <tr>
...
function renderScoreSheetSuerteCell(canonicalTotals = {}, suerteId = "", options = {}) {
  ...
  if (options.includeInfractions === false) return html`<td class="num">${moneylessNumber(official.total)}</td>`;
  return html`
    <td class="num">
...
function getCompactScoreSheetSuerteLabel(suerte = {}) {
  const labels = {
    cala: "C",
    piales: "P",
    colas: "C",
  ...
  return labels[suerte.id] || suerte.name || "—";
}
...
function getCanonicalResultsTeamTotals(charreadaId = "", teamId = "") {
  const charreada = state.charreadas.find((item) => item.id === charreadaId) || null;
  const tournamentId = charreada?.tournamentId || state.activeTournamentId || "";
  return getCanonicalOfficialTeamTotals({
    publishedScores: (state.publishedScores || []).filter((item) => (
      (item.tournament?.id || item.tournamentId || "") === tournamentId
    )),
...
function getLiveScreens() {
  return [
    ...
    {
      id: "coleadero-torneo",
      label: "Coleadero torneo",
      group: "obs",
      fileName: "grafico-coleadero-torneo.html",
      href: getGraphicHref("grafico-coleadero-torneo.html"),
      absoluteHref: getAbsoluteGraphicHref("grafico-coleadero-torneo.html")
    },
    {
      label: "Categoria",
      group: "obs",
...
function renderLiveScreenUrl(screen) {
  const id = `live-url-${String(screen.id || screen.fileName).replace(/[^a-z0-9]/gi, "-")}`;
  return html`
...
function buildScoringAttemptUiModel(context) {
  ...
  const overlappingTimerRuntime = getOverlappingCurrentTimerRuntime(context, fmchSportTimers[0]?.timer?.timerId);
  ...
  return buildScorerAttemptViewModel(attemptV2, {
    ...
    timers: ternaRuntime
      ...
      : overlappingTimerRuntime
        ? [overlappingTimerRuntime, ...fmchSportTimers].map((runtime, index) => ({
  ...
  return html`
    <div class="cp-context-selector cp-terna-opportunity-bar" data-scorer-zone="opportunity-selector" data-terna-session="${escapeHTML(runtime.session.ternaSessionId)}">
      <div class="opportunity-summary">
        ...
        <em>${current ? `Actual ${current}` : "Agotadas"}</em>
      </div>
      <div class="opportunity-controls cp-terna-opportunities" aria-label="Cinco oportunidades compartidas de Terna">
        ${runtime.session.opportunities.map((opportunity) => {
          ...
          return html`
            ...
            >${opportunity.sharedSequenceNumber}</button>
          `;
        }).join("")}
      </div>
    </div>
...
function renderManganasMainPanel(context) {
  ...
  const technicalRemate = normalizeFmch2026ManganaRemateIdentity(attempt.manganaRemate);
  const timeSettlement = getFmch2026ManganaFaenaTimeSettlement(attempts);
  ...
  const manualAdditionalTotal = Number(attempt.manganaManualAdditionalTotal ?? attempt.floreoTotal ?? 0);
  const remateScoringEffects = isPie
    ...
    <section class="cp-scoring-card cp-main-suerte-panel cp-manganas-panel" data-suerte="${escapeHTML(context.suerte.id)}">
      ...
      <div class="cp-manganas-dashboard">
        ...
        <div class="cp-mangana-result" role="group" aria-label="Resultado deportivo de la oportunidad">
          <span>Resultado</span>
          <button type="button" class="button ${attempt.manganaPlacedAt ? "active" : ""}" data-action="set-mangana-placed">Mangana puesta</button>
        ...
        <article class="cp-mangana-floreo" data-source="FMCH_2026">
          ...
          <div class="cp-sport-stepper">
            <button type="button" data-action="adjust-mangana-floreo" data-delta="-1" aria-label="Restar un punto adicional">−</button>
            <strong>${moneylessNumber(manualAdditionalTotal)}</strong>
            <button type="button" data-action="adjust-mangana-floreo" data-delta="1" aria-label="Sumar un punto adicional">+</button>
          </div>
          <em>${attempt.floreoScoredTotal === manualAdditionalTotal ? "Captura rápida autorizada" : "Centenario: adicionales no puntuables"}</em>
        </article>
        ...
        <article class="cp-mangana-time-settlement">
          <span>Adicional por tiempo de la faena</span>
          <strong>${timeSettlement ? `+${moneylessNumber(timeSettlement.points)}` : "Sin liquidar"}</strong>
          <em>${timeSettlement ? "Liquidación única del cronómetro oficial" : "Se aplica una sola vez para las tres oportunidades"}</em>
        </article>
      </div>
      <section class="cp-mangana-remates cp-mangana-technical-remate">
        <span>Identidad técnica del remate</span>
        ${isPie ? html`
          <p class="cp-mangana-remate-status">${technicalRemate
            ? `Registrado: ${escapeHTML(technicalRemate.name)}`
            : "Selecciona un atajo o usa la captura técnica. Puede registrar 0 puntos adicionales."}</p>
          <div class="cp-mangana-remate-shortcuts" role="group" aria-label="Atajos de identidad técnica del remate">
            ${MANGANAS_PIE_REMATE_CAPTURE_SHORTCUTS.map((shortcut) => html`
              <button
                type="button"
                class="button ${isManganaRemateShortcutSelected(technicalRemate, shortcut) ? "primary" : ""}"
                data-action="select-mangana-remate-shortcut"
                data-id="${escapeHTML(shortcut.id)}"
              >${escapeHTML(shortcut.label)}</button>
            `).join("")}
            <button type="button" class="button" data-action="focus-mangana-remate-custom">Otro / captura técnica</button>
          </div>
          <div class="cp-mangana-remate-form">
            <label>Efecto final<input id="mangana-remate-effect" maxlength="120" value="${escapeHTML(technicalRemate?.effectFinal || "")}" placeholder="Ej. Rodada"></label>
            <label>Orientación
              <select id="mangana-remate-orientation">
                <option value="">Seleccionar</option>
                <option value="MASK" ${technicalRemate?.orientation === "MASK" ? "selected" : ""}>Máscara</option>
                <option value="COUNTER_MASK" ${technicalRemate?.orientation === "COUNTER_MASK" ? "selected" : ""}>Contra máscara</option>
              </select>
            </label>
            <label>Sentido de vueltas
              <select id="mangana-remate-turn-direction">
                <option value="">Seleccionar</option>
                <option value="SAME" ${technicalRemate?.turnDirection === "SAME" ? "selected" : ""}>Mismo sentido</option>
                <option value="OPPOSITE" ${technicalRemate?.turnDirection === "OPPOSITE" ? "selected" : ""}>Sentido contrario</option>
              </select>
            </label>
            <label>Posición o movimiento final, opcional<input id="mangana-remate-body-finish" maxlength="120" value="${escapeHTML(technicalRemate?.bodyFinish || "")}" placeholder="Detalle breve"></label>
            <button type="button" class="button primary" data-action="save-mangana-remate">Guardar remate</button>
          </div>
        ` : html`
          <p>${technicalRemate ? `${escapeHTML(technicalRemate.name)} · identidad ${escapeHTML(technicalRemate.signature)}` : "Selecciona la base reglamentaria para identificar el remate."}</p>
        `}
      </section>
      ${isPie ? html`
        <section class="cp-mangana-remates cp-mangana-remate-effects">
          <span>Adicionales al rematar</span>
          <div>
            ${remateScoringEffects.map((effect) => html`<button type="button" class="button ${attempt.applied?.includes(effect.id) ? "primary" : ""}" data-action="toggle-rule" data-type="adic" data-id="${escapeHTML(effect.id)}">${escapeHTML(effect.label)} +${moneylessNumber(effect.pts)}</button>`).join("")}
          </div>
        </section>
      ` : ""}
...
function formatManganaHistoryDetail(attempt = {}, suerte = {}) {
  ...
  const technicalRemate = normalizeFmch2026ManganaRemateIdentity(attempt.manganaRemate);
  if (technicalRemate) return technicalRemate.name;
...
function renderOfficialSportTimer(runtime) {
  ...
  return html`
    <article class="cp-sport-timer ${view.paused ? "paused" : ""}" data-timer-kind="${escapeHTML(runtime.kind)}">
      ...
      <em>${escapeHTML(officialTimerDisplayStateLabel(view.status))}${view.pauseReason ? ` · ${view.pauseReason}` : ""} · Control: ${escapeHTML(control.controllerLabel)}</em>
    ...
    <section class="cp-scoring-card cp-main-suerte-panel cp-jineteo-panel">
      ...
      <div class="cp-jineteo-controls cp-operational-control-bar">
        <div class="cp-jineteo-timing-summary">
          ...
          <strong>${escapeHTML(officialTimerDisplayStateLabel(timerView.status || "READY"))}</strong>
...
function getOverlappingCurrentTimerRuntime(context, localTimerId = "") {
  const current = state.currentTimerContext;
  if (!current?.timerId || current.timerId === localTimerId) return null;
  if (context?.suerte?.id !== "toro" || current.suerteId !== "terna") return null;
  const timer = state.officialTimers?.[current.timerId];
  if (!timer) return null;
  return {
    kind: "orchestrated_current",
    label: current.suerteLabel || current.phaseLabel || "Terna en el Ruedo",
    definition: current,
    timer,
    view: getOfficialTimerContextView(timer, { definition: current })
  };
}
    ...
    <section class="cp-scoring-card cp-main-suerte-panel cp-terna-panel" data-terna-status="${escapeHTML(session.status)}">
      ...
      <div class="cp-terna-dashboard">
        ...
        <article class="cp-terna-counted cp-terna-time-reference">
          ...
          <strong>${escapeHTML(officialTimerDisplayStateLabel(timer.status))}</strong>
        ...
        <div class="cp-paso-primary">
          ...
          <div class="cp-paso-timer-controls" aria-label="Controles temporales del Paso">
            ${timers.map((runtime) => html`
              <section>
                <div><span>${escapeHTML(runtime.kind === "paso_exit" ? "Salida" : "Desmonte")}</span><strong>${escapeHTML(officialTimerDisplayStateLabel(runtime.view.status))}</strong></div>
...
function renderScoringActionAccordions(charreada, context, charroName, leaderboard) {
  ...
  return [
    ...
    renderScoringAccordionGroup("infr", "Infracciones", "warning", renderScoringActionGroupBody("infr", infrButtons, charreada, context, charroName, leaderboard), { collapsible: true, defaultOpen: true }),
...
function renderScoringButtonLayout(groupId, buttons, context) {
  if (groupId === "adic" && context.suerte.id === "cala") {
    return renderNamedScoringGroups(CALA_ADIC_SECTIONS.map((section) => ({
      ...
      renderMode: section.code === "ML" ? "cala_medios_lados" : "default"
    })), buttons);
  }
    ...
    <div class="cp-scoring-family-groups">
      ${normalized.map((group) => html`
        <section class="cp-scoring-family-group">
          ...
          ${group.renderMode === "cala_medios_lados"
            ? renderCalaMediosLadosAdditionalGroup(group.buttons)
            : html`<div class="cp-scoring-button-grid">${group.buttons.map((button) => renderConfigurableScoringButton(button)).join("")}</div>`}
        </section>
      `).join("")}
    </div>
  `;
...
function renderCalaMediosLadosAdditionalGroup(buttons) {
  const byRuleId = new Map(buttons.map((button) => [button.ruleId, button]));
  const sides = [
    {
      label: "MEDIO LADO DERECHO",
      controls: [
        ["Ida", FMCH_2026_CALA_MEDIOS_LADOS_RULE_IDS.RIGHT_OUTBOUND],
        ["Vuelta", FMCH_2026_CALA_MEDIOS_LADOS_RULE_IDS.RIGHT_RETURN]
      ]
    },
    {
      label: "MEDIO LADO IZQUIERDO",
      controls: [
        ["Ida", FMCH_2026_CALA_MEDIOS_LADOS_RULE_IDS.LEFT_OUTBOUND],
        ["Vuelta", FMCH_2026_CALA_MEDIOS_LADOS_RULE_IDS.LEFT_RETURN]
      ]
    }
  ];
  const hasGranularContract = sides.every((side) =>
    side.controls.every(([, ruleId]) => byRuleId.has(ruleId))
  );
  if (!hasGranularContract) {
    return html`<div class="cp-scoring-button-grid">${buttons.map((button) => renderConfigurableScoringButton(button)).join("")}</div>`;
  }
  return html`
    <div class="cp-cala-medios-lados-additional" aria-label="Identidades técnicas de medios lados">
      ${sides.map((side) => html`
        <section class="cp-cala-medio-lado-side">
          <h4>${side.label}</h4>
          <div class="cp-cala-medio-lado-actions">
            ${side.controls.map(([label, ruleId]) => renderConfigurableScoringButton({
              ...byRuleId.get(ruleId),
              shortLabel: label,
...
function renderScoringAccordionGroup(groupId, title, icon, body, options = {}) {
  ...
  return html`
    <section class="cp-accordion-group open cp-fixed-action-block" data-accordion="${groupId}" data-scorer-zone="sport-controls">
  ...
  return html`
    <div class="cp-header-timer-group" data-timer-count="${attemptView.timers.length}">
      ${attemptView.timers.map((timer) => {
        ...
        const timerDataset = isTernaTimer
          ? `data-terna-timer-id="${escapeHTML(timer.timerId || "")}"`
          : isOfficialSportTimer ? `data-official-timer-id="${escapeHTML(timer.timerId || "")}"` : "";
        return html`
          <section class="header-timer read-only cp-timer-tile ${timer.primary ? "primary" : ""}">
            <span>${escapeHTML(timer.label)}</span>
            <strong class="${displayClass}" ${timerDataset}>${escapeHTML(timer.display)}</strong>
...
function renderScoringLiveOverview(attemptView) {
  const summary = attemptView.summary;
  return html`
    <section class="cp-live-score-overview" data-scorer-zone="score-overview" aria-label="Puntuacion y tiempo actuales">
      <div class="cp-live-total">
        <span>Total actual</span>
        <strong>${moneylessNumber(summary.totalPoints)} <small>pts</small></strong>
        <em class="status-${escapeHTML(attemptView.status.tone)}">${escapeHTML(attemptView.status.label)}</em>
      </div>
      <div class="cp-live-breakdown" aria-label="Desglose actual">
        <span><small>Base</small><strong>${moneylessNumber(summary.basePoints)}</strong></span>
        <span><small>Adic.</small><strong>+${moneylessNumber(summary.additionalPoints)}</strong></span>
        <span><small>Infr.</small><strong>-${moneylessNumber(summary.individualBadPoints)}</strong></span>
      </div>
...
function hasAttemptScoringActivity(attempt = {}) {
  return Boolean(
    ...
    Number(attempt.manganaManualAdditionalTotal ?? attempt.floreoTotal ?? 0) ||
...
function renderScoringHeader(charreada, context, charroName, attemptView) {
  ...
  return html`
    <header class="scoring-header" data-scorer-zone="context">
      <button class="button" data-action="exit-scoring">Salir</button>
      <div class="scoring-livebar">
        <div class="cp-judge-context">
          <div class="scoring-title">
            <p>Calificando en vivo ${renderFirebaseRuntimeBadge()}</p>
            <h1>${escapeHTML(context.suerte.fullName)}</h1>
            <small>${escapeHTML(context.tournament?.name || "CharroPro")} · ${escapeHTML(charreada.name)}</small>
          </div>
          <dl class="cp-judge-context-grid">
            <div><dt>${escapeHTML(labels.scoreEntityLabel)}</dt><dd>${escapeHTML(getEntryDisplayName(context.team) || "Sin registrar")}</dd></div>
            <div><dt>Participante</dt><dd>${escapeHTML(charroName || "Sin registrar")}</dd></div>
            ${horseName ? html`<div><dt>Caballo</dt><dd>${escapeHTML(horseName)}</dd></div>` : ""}
            <div><dt>Oportunidad</dt><dd>${opportunity.number}/${opportunity.total} · ${escapeHTML(attemptView.status.label)}</dd></div>
          </dl>
...
function renderTimeNoteSection(context) {
  ...
  return html`
    <details class="card time-evidence-card cp-inline-secondary-control" data-scorer-zone="secondary-controls">
      <summary class="card-header">
        <strong>Evidencia y nota</strong>
        <span>${evidence.length ? `${evidence.length} registro(s)` : "Opcional"}</span>
      </summary>
      <div class="card-body form-grid">
        <div class="wide cp-time-evidence-actions">
...
function handleAction(action, target) {
  ...
  const handlers = {
    ...
    "set-mangana-placed": captureManganaPlaced,
    "set-paso-result": () => applyPasoResult(target.dataset.result),
    "select-mangana-remate-shortcut": () => selectManganaRemateShortcut(target.dataset.id),
    "focus-mangana-remate-custom": focusManganaRemateCustom,
    "save-mangana-remate": saveManganaTechnicalRemate,
    ...
    "run-historical-reconciliation-dry-run": runHistoricalReconciliationDryRun,
...
async function confirmDeleteTournament(tournamentId) {
  ...
  if (Array.isArray(preflight.blockingReasons) && preflight.blockingReasons.length) {
    showModal({
      ...
      body: html`
        <p>Este torneo oficial no puede eliminarse definitivamente porque contiene historial.</p>
        <p class="card-subtitle">La version comercial permite conservarlo mediante Congelar/Archivar.</p>
      `,
  ...
  showModal({
    ...
    body: html`
      ...
      <p><strong>${preflight.dataClassification === "TEST" ? "Torneo de prueba — eliminacion definitiva permitida." : "Torneo sin historial oficial — eliminacion definitiva permitida."}</strong></p>
...
async function saveTournament() {
  ...
  let releaseConfiguration = null;
  const liveConfigured = isFirebaseLiveConfigured();
  if (liveConfigured) {
    try {
      releaseConfiguration = await readFirebaseConfiguration({});
    } catch (error) {
      tournamentCreationInProgress = false;
      showToast(`No se pudo resolver la autoridad de release: ${error?.message || "configuration-unavailable"}`);
      return;
    }
  }
  const tournament = applyReleaseClassificationToNewTournament(applyDefaultPublicPortalModules(applyProductiveRuleProfilePolicy(applyLocalFmch2026RuleProfileDefault({
    id,
    name: data.name.trim(),
  ...
  }, getFirebaseRuntimeDiagnostics()))), releaseConfiguration || undefined);
...
function reconcileManganaRemateConstraint(context) {
  ...
  const currentRemate = normalizeFmch2026ManganaRemateIdentity(context.attempt.manganaRemate);
  const repeated = currentRemate
    ? attempts.slice(0, context.attemptIndex).some((attempt) => (
        normalizeFmch2026ManganaRemateIdentity(attempt?.manganaRemate)?.signature === currentRemate.signature
      ))
    : buildFmch2026ManganaRemateHistory(attempts.slice(0, context.attemptIndex))
        .some((item) => item.remateId === context.attempt.remateId);
  if (repeated) {
  ...
  if (type === "base") {
    ...
    if (!wasActive) {
      ...
      if (context.suerte.id === "piales" || isFmch2026TernaSuerte(context.suerte.id)) {
        ...
        Object.assign(context.attempt, applyFmch2026ManganaTechnicalRemate(context.attempt, context.suerte, {
          shortcutId: rule.id
        }));
      ...
      if (context.suerte.id === "manganas_caballo") context.attempt.manganaRemate = null;
    }
  } else {
...
function buildManganaFaenaIdentity(context, runtime) {
  const individual = Boolean(context.competitionContext?.isIndividualCompetition);
  return {
    tournamentId: context.tournament?.id || "",
    competitionId: context.charreada?.competitionId || context.competitionContext?.competitionId || "equipos_completo",
    charreadaId: context.charreada?.id || "",
    teamId: individual ? "" : context.team?.id || "",
    participantId: individual ? context.participant?.id || context.team?.id || "" : "",
    suerteId: context.suerte?.id || "",
    timerId: runtime?.timer?.timerId || ""
  };
}

function settleCurrentManganaFaena(context, runtime) {
  const result = settleFmch2026ManganaFaenaTime(getAttemptsForContext(context), context.suerte, {
    ...buildManganaFaenaIdentity(context, runtime),
  ...
  const key = scoreKey(context.charreada.id, context.team.id, context.suerte.id);
  state.scores[key] = result.attempts;
  context.attempt = result.attempts[context.attemptIndex] || context.attempt;
  return result;
}
...
function freezeCurrentFmchSportAttempt(context) {
  ...
  if (context.suerte?.id === "cala") {
    const runtime = runtimes.find((item) => item.kind === "cala_partidero");
    if (runtime) {
      Object.assign(context.attempt, applyFmch2026CalaPartideroTiming(context.attempt, {
        timerId: runtime.timer.timerId,
        officialElapsedMs: runtime.view.officialElapsedMs,
        durationMs: runtime.view.durationMs,
        status: runtime.view.status
      }));
      reconcileAttemptConditionalRules(context);
    }
  ...
  if (context.suerte?.id === "piales") {
    ...
    if (runtime) {
      Object.assign(context.attempt, applyFmch2026PialesTiming(context.attempt, {
        timerId: runtime.timer.timerId,
        officialElapsedMs: runtime.view.officialElapsedMs,
        durationMs: runtime.view.durationMs,
        status: runtime.view.status
  ...
  if (isFmch2026JineteoSuerte(context.suerte?.id) && context.attempt.classification?.classificationId) {
    const runtime = runtimes[0];
    if (runtime) {
      const dismount = context.suerte.id === "yegua"
        ? runtimes.find((item) => item.phaseId === "dismount")
        : null;
      Object.assign(
        context.attempt,
  ...
  if (isFmch2026ManganaSuerte(context.suerte?.id)) {
    ...
    const runtime = runtimes[0];
    if (runtime) {
      const sequenceComplete = context.attemptIndex >= FMCH_2026_MANGANAS_OPPORTUNITY_LIMIT - 1
        || runtime.view.status === "FINISHED";
      if (sequenceComplete) {
        settleCurrentManganaFaena(context, runtime);
      } else {
        Object.assign(context.attempt, applyFmch2026ManganaAttemptTiming(context.attempt, context.suerte, {
          timerId: runtime.timer.timerId,
          officialElapsedMs: runtime.view.officialElapsedMs,
          wallElapsedMs: runtime.view.wallElapsedMs,
          status: runtime.view.status,
      ...
      context.attempt.timing.startedAt = runtime.timer.wallStartedAt;
      context.attempt.timing.endedAt = runtime.timer.wallFinishedAt;
    }
    Object.assign(context.attempt, reconcileFmch2026ManganaAttempt(context.attempt, context.suerte));
  }
  if (isFmch2026PasoSuerte(context.suerte?.id)) {
    ...
    if (exit && dismount) {
      Object.assign(context.attempt, applyFmch2026PasoTiming(context.attempt, context.suerte, {
        ...
        dismountOfficialElapsedMs: dismount.view.officialElapsedMs,
        exitExpired: exit.view.officialElapsedMs > FMCH_2026_PASO_EXIT_DURATION_MS,
        secondaryTimers: [{
      ...
      context.attempt.timing.wallElapsedMs = exit.view.wallElapsedMs;
      context.attempt.timing.startedAt = exit.timer.wallStartedAt;
      context.attempt.timing.endedAt = exit.timer.wallFinishedAt;
    }
    Object.assign(context.attempt, reconcileFmch2026PasoAttempt(context.attempt, context.suerte));
  }
}
...
function buildPublishedScoreSnapshot(context) {
  ...
  if (context.suerte?.id === "cala") {
    const frozenTotal = Number(attempt.puntaPts || 0);
    applyPuntaCalculation(attempt);
    if (attempt.puntaPts !== frozenTotal) throw new Error("cala-punta-evidence-total-mismatch");
  }
...
function buildPublishedBreakdown(context, attempt, publication = {}) {
  ...
  return {
    ...
    puntaPuntosDistancia: punta?.puntosDistancia ?? null,
    puntaPuntosTiempos: punta?.puntosTiempos ?? null,
    punta,
...
function adjustManganaFloreo(delta) {
  ...
  Object.assign(
    ...
    setFmch2026ManganaManualAdditionalTotal(
      context.attempt,
      context.suerte,
      Number(context.attempt.manganaManualAdditionalTotal ?? context.attempt.floreoTotal ?? 0) + Number(delta || 0)
    )
  );
  persistScoreChange();
}
...
function applyManganaResult(result) {
  ...
  context.attempt.manganaScoringContractVersion = FMCH_2026_MANGANA_SCORING_CONTRACT_VERSION;
  if (result === "ACHIEVED") {
    const runtime = getFmch2026SportTimerRuntimes(context)[0];
    if (runtime) {
      Object.assign(context.attempt, recordFmch2026ManganaEventEvidence(context.attempt, "DOWN", {
        timerId: runtime.timer.timerId,
        officialElapsedMs: runtime.view.officialElapsedMs
      }));
    }
...
function captureManganaPlaced() {
  if (!guardUnlockedCharreada()) return;
  const context = getCurrentContext();
  if (!context?.attempt || !isFmch2026ManganaSuerte(context.suerte?.id)) return;
  const runtime = getFmch2026SportTimerRuntimes(context)[0];
  if (!runtime) {
    showToast("El cronómetro oficial de la faena no está disponible.");
    return;
  }
  Object.assign(context.attempt, recordFmch2026ManganaEventEvidence(context.attempt, "PLACED", {
    timerId: runtime.timer.timerId,
    officialElapsedMs: runtime.view.officialElapsedMs
  }));
  persistScoreChange();
...
function saveManganaTechnicalRemate() {
  if (!guardUnlockedCharreada()) return;
  const context = getCurrentContext();
  if (!context?.attempt || context.suerte?.id !== "manganas_pie") return;
  const input = {
    name: document.getElementById("mangana-remate-effect")?.value || "",
    effectFinal: document.getElementById("mangana-remate-effect")?.value || "",
    orientation: document.getElementById("mangana-remate-orientation")?.value || "",
    turnDirection: document.getElementById("mangana-remate-turn-direction")?.value || "",
    bodyFinish: document.getElementById("mangana-remate-body-finish")?.value || ""
  };
  const next = applyFmch2026ManganaTechnicalRemate(context.attempt, context.suerte, input);
  const validation = validateFmch2026ManganaRemateIdentity(next.manganaRemate);
  if (!validation.valid) {
    showToast("Completa efecto final, orientación y sentido de vueltas del remate.");
    return;
  }
  Object.assign(context.attempt, next);
  reconcileManganaRemateConstraint(context);
  persistScoreChange();
}

const MANGANAS_PIE_REMATE_CAPTURE_SHORTCUTS = Object.freeze([
  Object.freeze({ id: "capture_rodada", label: "Rodada", effectFinal: "Rodada", orientation: "MASK", turnDirection: "SAME" }),
  Object.freeze({ id: "capture_bigotona", label: "Bigotona", effectFinal: "Bigotona", orientation: "MASK", turnDirection: "SAME" }),
  Object.freeze({ id: "capture_contra_rodada", label: "Contra rodada", effectFinal: "Rodada", orientation: "COUNTER_MASK", turnDirection: "OPPOSITE" })
]);

function isManganaRemateShortcutSelected(remate, shortcut) {
  return Boolean(remate
    && shortcut
    && remate.effectFinal === shortcut.effectFinal
    && remate.orientation === shortcut.orientation
    && remate.turnDirection === shortcut.turnDirection);
}

function selectManganaRemateShortcut(shortcutId) {
  if (!guardUnlockedCharreada()) return;
  const context = getCurrentContext();
  if (!context?.attempt || context.suerte?.id !== "manganas_pie") return;
  const shortcut = MANGANAS_PIE_REMATE_CAPTURE_SHORTCUTS.find((item) => item.id === shortcutId);
  if (!shortcut) return;
  Object.assign(context.attempt, applyFmch2026ManganaTechnicalRemate(context.attempt, context.suerte, {
    name: shortcut.label,
    effectFinal: shortcut.effectFinal,
    orientation: shortcut.orientation,
    turnDirection: shortcut.turnDirection
  }));
  reconcileManganaRemateConstraint(context);
  persistScoreChange();
}

function focusManganaRemateCustom() {
  document.getElementById("mangana-remate-effect")?.focus();
}
...
async function applyFmchSportTimerCommand(timerKind, command) {
  ...
  if (result.ok && command === "FINISH") {
    await prepareTernaCurrentTimerAfterToroFinish(result.timer || runtime.timer, context, getScorerTimerController(), getAccessActor());
  }
}

async function prepareTernaCurrentTimerAfterToroFinish(finishedTimer, context, controller, actor) {
  const source = {
    tournament: context?.tournament,
    charreada: context?.charreada,
    turn: {
  ...
  const definition = buildToroToTernaReadyDefinition(source, finishedTimer);
  if (!definition) return { ok: true, skipped: true };
  const existing = state.officialTimers?.[definition.timerId] || null;
  const result = await applyFirebaseOfficialTimerAuthority(definition, {
    operation: "CLAIM_CONTROL",
    timerId: definition.timerId,
    commandId: createScorerTimerCommandId(definition.timerId, "PREPARE_TERNA_READY"),
  ...
  if (result.timer) setOfficialTimer(result.timer);
  if (result.ok) {
    state.currentTimerContext = resolveOfficialCurrentTimerContext({
      source,
      registry: state.officialTimers,
      currentTimerContext: buildCurrentContextFromAuthorityResult(result, definition, finishedTimer.timerId),
...
function buildCurrentContextFromAuthorityResult(result, definition, handoffFromTimerId) {
  return buildOfficialCurrentTimerContext(result.timer || definition, definition, {
...
function getScorerTimerRuntime(timerId, context = getCurrentContext()) {
  ...
  const overlapping = getOverlappingCurrentTimerRuntime(context);
  if (overlapping?.timer?.timerId === timerId) return overlapping;
...
function applyCurrentFmchSportTiming() {
  ...
  if (isFmch2026ManganaSuerte(context.suerte?.id)) {
    ...
    if (sequenceComplete) {
      ...
      showToast("El adicional por tiempo se liquidará al cerrar la faena.");
    }
...
async function publishOfficialScoreForContext(context, options = {}) {
  if (isFmch2026ManganaSuerte(context?.suerte?.id)) {
    const collectionGuard = validateFmch2026ManganaOfficialCollection(
      getAttemptsForContext(context),
      context.suerte,
      buildManganaFaenaIdentity(context, null)
    );
    if (!collectionGuard.valid) {
      const detail = collectionGuard.errors.join(", ");
      setLastFirebaseError("manganas-official-collection-invalid", detail);
      setScoreSaveStatus({
        state: "error",
      ...
      showToast("La liquidación oficial de Manganas no es válida. Revisa tiempo y remates.");
      return { ok: false, reason: "manganas-official-collection-invalid", errors: collectionGuard.errors };
    }
  }
...
function reconcileCommittedOfficialScore(previousRecords = [], committed = {}) {
  ...
  const sportingOpportunityKey = getCanonicalSportingOpportunityKey(record);
  const recordId = String(record.id || "");
  const next = (Array.isArray(previousRecords) ? cloneJson(previousRecords) : [])
    .filter((entry) => String(entry?.id || "") !== recordId)
    .map((entry) => {
      if (!sportingOpportunityKey || getCanonicalSportingOpportunityKey(entry) !== sportingOpportunityKey || entry?.superseded) return entry;
      return {
        ...entry,
        superseded: true,
...
async function nextScore() {
  ...
  if (
    isFmch2026ManganaSuerte(context.suerte?.id)
    && context.attempt.manganaScoringContractVersion === FMCH_2026_MANGANA_SCORING_CONTRACT_VERSION
    && context.attempt.manganaResult === "ACHIEVED"
    && !validateFmch2026ManganaRemateIdentity(context.attempt.manganaRemate).valid
  ) {
    showToast("Registra la identidad técnica del remate antes de publicar la mangana lograda.");
    finalizeScorerSaveLatency(latencyTrace, "failed", { reason: "mangana-remate-required" });
    return;
  }
...
async function applyTernaTimeAdditionalIfEligible(context, sourceSession = null, sourceTimer = null) {
  ...
  for (const target of pendingTargets) {
    ...
    const correctionContext = {
      ...
      coleadorIndex: Math.max(0, Number(target.entry.participantSlot || 1) - 1)
    };

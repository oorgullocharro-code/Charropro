import { SUERTES, TOURNAMENT_TYPES, getTournamentSuertes, getTournamentTypeConfig } from "./data/suertes.js?v=20260622-prepare-gate1";
import {
  SCORING_BUTTON_GROUPS,
  normalizeScoringButtonGroup,
  normalizeScoringButtonLayouts
} from "./data/defaultScoringButtonLayouts.js?v=20260622-prepare-gate1";
import {
  CALA_ADIC_SECTIONS,
  ...
  sumTeamPenalties
} from "./data/calaRules.js?v=20260622-prepare-gate1";
import { closeModal, escapeHTML, html, moneylessNumber, showModal, showToast } from "./core/dom.js?v=20260622-prepare-gate1";
import { exportBackupJson, exportCurrentTournamentCsv } from "./core/exporters.js?v=20260622-prepare-gate1";
import { advanceScoringPointer, previousScoringPointer, resetScoringPointer } from "./core/flow.js?v=20260622-prepare-gate1";
import { downloadOfficialFormatXlsx } from "./core/officialFormat.js?v=20260622-prepare-gate1";
import { getTimerScopeKey, getTimerView } from "./core/timerRules.js?v=20260622-prepare-gate1";
import { buildStatisticalHistorySnapshot } from "./core/history.js?v=20260622-prepare-gate1";
import { buildCharroProStatsCenter } from "./core/statistics.js?v=20260622-prepare-gate1";
import {
...
import { ROLES, ROLE_OPTIONS, getRoleLabel, hasTournamentAccess, isActiveAccessSession, normalizeTournamentAccess, roleCan } from "./core/roles.js?v=20260622-prepare-gate1";
import {
  buildTournamentUrl,
...
} from "./core/tournamentContext.js?v=20260622-prepare-gate1";
import {
  createRoster,
  emptyAttempt,
  ...
  state,
  uid
} from "./core/state.js?v=20260622-prepare-gate1";

const app = document.getElementById("app");
const OBS_PAGE_VERSION = "20260624-supervisor-review1";
...
const SIDEBAR_STORAGE_KEY = "charropro_sidebar_collapsed";
const TURN_PANEL_STORAGE_KEY = "charropro_turn_panel_collapsed";
const TEAMS_TAB_STORAGE_KEY = "charropro_teams_tab";
const SCORING_ACCORDION_STORAGE_KEY = "charropro_scoring_accordion_state";
const FIREBASE_CLIENT_ID_KEY = "charropro_firebase_client_id";
const APP_CACHE_VERSION_STORAGE_KEY = "charropro_cache_version";
const PREPARE_STORAGE_KEY = "charropro_prepare_status_v1";
      ...
      <main class="main">
        <header class="topbar">
          ...
          <div class="topbar-actions">
            ${renderAccessWidget()}
            ${tournamentScoped
              ? html`${IS_TOURNAMENT_APP ? "" : renderTournamentPicker()}<a class="button" href="./index.html">Vista general</a>`
...
function getSidebarFooterLinks() {
  ...
  if (roleCan(role, "speaker")) {
    links.push({ label: "Locutores", href: "./locutores.html", icon: "mic" });
  }
...
function canShowNavView(view) {
  ...
  if (IS_TOURNAMENT_APP) {
    ...
    if (role === ROLES.ORGANIZADOR || role === ROLES.LECTURA) {
      return ["dashboard", "program", "results", "stats", "settings"].includes(view);
    }
    if (role === ROLES.JUEZ) return ["dashboard", "program"].includes(view);
    if (role === ROLES.GRAFICOS) return ["graphicsAccess", "settings"].includes(view);
    return false;
  }
  ...
  if (role === ROLES.GRAFICOS) return ["tournaments", "graphicsAccess", "settings"].includes(view);
  return false;
}
...
function readTurnPanelPreference() {
  try {
    return localStorage.getItem(TURN_PANEL_STORAGE_KEY) === "1";
  } catch {
    return false;
  }
}
...
function saveTurnPanelPreference() {
  try {
    localStorage.setItem(TURN_PANEL_STORAGE_KEY, turnPanelCollapsed ? "1" : "0");
  } catch {
    // La preferencia visual no debe bloquear el uso de la app.
  }
}
...
function readTeamsTabPreference() {
  try {
    return localStorage.getItem(TEAMS_TAB_STORAGE_KEY) || "quick";
  } catch {
    return "quick";
  }
}
...
function saveTeamsTabPreference() {
  try {
    localStorage.setItem(TEAMS_TAB_STORAGE_KEY, teamsTab);
  } catch {
    // La preferencia visual no debe bloquear el uso de la app.
  }
}
...
function readScoringAccordionPreference() {
  ...
  try {
    const saved = sessionStorage.getItem(SCORING_ACCORDION_STORAGE_KEY);
...
function saveScoringAccordionPreference() {
  try {
    sessionStorage.setItem(SCORING_ACCORDION_STORAGE_KEY, JSON.stringify(scoringAccordionState));
  } catch {
...
function readFirebaseClientId() {
  try {
    const saved = localStorage.getItem(FIREBASE_CLIENT_ID_KEY);
    if (saved) return saved;
    const created = uid("cliente");
    localStorage.setItem(FIREBASE_CLIENT_ID_KEY, created);
    return created;
  } catch {
    return uid("cliente");
  }
...
function clearStaleLocalTournamentCache() {
  try {
    const savedVersion = localStorage.getItem(APP_CACHE_VERSION_STORAGE_KEY);
    ...
    localStorage.setItem(APP_CACHE_VERSION_STORAGE_KEY, OBS_PAGE_VERSION);
    saveState({ silent: true });
  } catch {
...
function clearLocalCacheAndReload() {
  try {
    ...
    localStorage.removeItem(LIVE_TIMER_KEY);
    localStorage.removeItem(APP_CACHE_VERSION_STORAGE_KEY);
  } catch (error) {
...
function readPrepareState() {
  try {
    return JSON.parse(sessionStorage.getItem(PREPARE_STORAGE_KEY) || "{}") || {};
  } catch {
    return {};
  }
}
...
function savePrepareState() {
  try {
    sessionStorage.setItem(PREPARE_STORAGE_KEY, JSON.stringify(prepareState || {}));
  } catch {
...
function readFirebaseDiagnostics() {
  try {
    return JSON.parse(localStorage.getItem(`${PREPARE_STORAGE_KEY}_diagnostics`) || "{}") || {};
  } catch {
    return {};
  }
}
...
function saveFirebaseDiagnostics() {
  try {
    localStorage.setItem(`${PREPARE_STORAGE_KEY}_diagnostics`, JSON.stringify(firebaseDiagnostics || {}));
  } catch {
...
function applyPreparationSnapshot(snapshot = {}) {
  ...
  Object.values(snapshot.tournaments || {}).forEach((payload) => {
    ...
    upsertStateItem(state.tournaments, remoteState.tournament);
  ...
  firebaseDiagnostics = buildPreparationDiagnostics(snapshot);
  saveFirebaseDiagnostics();
}

function buildPreparationDiagnostics(snapshot = {}) {
  const remoteCounts = {};
...
function startFirebaseAppStateSubscription() {
  ...
  if (IS_TOURNAMENT_APP) {
    ...
    if (!tournamentId || firebaseTournamentStateId === tournamentId && firebaseTournamentStateUnsubscribe) return;
    if (firebaseTournamentStateUnsubscribe) firebaseTournamentStateUnsubscribe();
...
function replaceTournamentScores(tournamentId, nextScores) {
...
function canAccessCurrentView() {
  ...
  if (IS_TOURNAMENT_APP) {
    if (role === ROLES.SUPERVISOR || role === ROLES.OPERADOR) return true;
    if (role === ROLES.ORGANIZADOR || role === ROLES.LECTURA) {
      return ["dashboard", "program", "results", "stats", "settings"].includes(state.view);
    }
    if (role === ROLES.JUEZ) return ["dashboard", "program", "scoring"].includes(state.view);
    if (role === ROLES.GRAFICOS) return ["graphicsAccess", "settings"].includes(state.view);
    return false;
  }
  if (role === ROLES.SUPERVISOR) return true;
  ...
  if (role === ROLES.GRAFICOS) return ["tournaments", "graphicsAccess", "settings"].includes(state.view);
  return false;
}
  ...
  return html`
    <section class="content tournament-entry">
      <article class="tournament-entry-hero">
        ...
        <button class="button primary" data-action="new-tournament">Crear torneo nuevo</button>
      </article>
      ${
        tournaments.length
          ? html`
          ...
          : html`
              <div class="empty tournament-empty">
                ...
                ${emptyState.action === "create" ? html`<button class="button primary" data-action="new-tournament">Crear primer torneo</button>` : ""}
...
function getTournamentEntryEmptyState() {
  ...
  return {
    title: "No tienes torneos activos asignados",
...
function renderNoAssignedTournaments() {
  return html`
    <section class="content">
      <div class="empty">
        <h2>No tienes torneos activos o programados asignados</h2>
...
function renderDashboard() {
  ...
  const leaderboard = buildLeaderboard(tournament.id);
  const activeCharreada = getActiveCharreada();
  const typeConfig = getTournamentTypeConfig(tournament.type);
  const labels = getEntityLabels(tournament);
  const locked = isTournamentFrozen(tournament);
  const activeLocked = isCharreadaFrozen(activeCharreada);
        ...
        <article class="card">
          <div class="card-header">
            <div>
              ...
              <p class="card-subtitle">Crear, programar y abrir el calificador.</p>
            </div>
          </div>
          <div class="card-body grid">
            ...
            <button class="button" data-action="new-charreada" ${locked ? "disabled" : ""}>Crear charreada</button>
            <button class="button green" data-action="start-scoring" ${activeCharreada && !activeLocked ? "" : "disabled"}>
              ${activeLocked ? "Calificacion bloqueada" : "Calificar charreada activa"}
            </button>
            <div class="segmented">
              ${charreadas
                .map(
                  (charreada) => html`
                    <button data-action="set-active-charreada" data-id="${charreada.id}" class="${charreada.id === state.activeCharreadaId ? "active" : ""}">
  ...
  return html`
    <section class="content program-page">
      <article class="program-hero">
        ...
        <button class="button primary" data-action="new-charreada" ${teams.length && !locked ? "" : "disabled"}>Nueva charreada</button>
      </article>
      ${
        charreadas.length
          ? renderCharreadasCards(charreadas)
          : html`
              <div class="empty program-empty">
	                ...
	                <button class="button primary" data-action="${teams.length ? "new-charreada" : "new-team"}" ${locked ? "disabled" : ""}>${teams.length ? "Crear primera charreada" : labels.add}</button>
              </div>
            `
      }
    </section>
...
function renderCharreadasCards(charreadas) {
  return html`
    <div class="program-card-grid">
      ${charreadas.map(renderCharreadaProgramCard).join("")}
    </div>
  `;
}

function renderCharreadaProgramCard(charreada) {
  const isActive = charreada.id === state.activeCharreadaId;
  const statusClass = getCharreadaStatusClass(charreada.status);
  ...
  return html`
    <article class="program-card ${isActive ? "active" : ""} ${locked ? "locked" : ""}">
      <div class="program-card-main">
        <div class="program-card-top">
          ...
          <div class="program-card-badges">
            ...
            <span class="pill ${statusClass}">${escapeHTML(formatCharreadaStatus(charreada.status))}</span>
          </div>
        </div>
      ...
      <div class="program-card-actions">
        <button class="button small" data-action="set-active-charreada" data-id="${charreada.id}" ${isActive ? "disabled" : ""}>Activar</button>
        <button class="button small" data-action="edit-charreada" data-id="${charreada.id}" ${locked ? "disabled" : ""}>Editar</button>
        <button class="button primary small" data-action="start-scoring" data-id="${charreada.id}" ${locked ? "disabled" : ""}>Calificar</button>
        <button class="button red small" data-action="delete-charreada" data-id="${charreada.id}" ${locked ? "disabled" : ""}>Eliminar</button>
      </div>
    </article>
  `;
}
        ...
        return html`
          <div class="program-summary-row ${isActive ? "active" : ""}">
            ...
            <div class="program-summary-actions">
              ${isActive ? html`<span class="pill blue">Activa</span>` : html`<button class="button small" data-action="set-active-charreada" data-id="${charreada.id}">Activar</button>`}
              <button class="button primary small" data-action="start-scoring" data-id="${charreada.id}" ${isCharreadaFrozen(charreada) ? "disabled" : ""}>Calificar</button>
            </div>
          </div>
        `;
      }).join("")}
...
function renderScoringBottomBar(context) {
  ...
  return html`
    <footer class="cp-bottom-bar">
      ...
      <div class="cp-bottom-sync">
        <span class="cp-sync-dot"></span>
        <strong>Sincronizado</strong>
        <em>Turno ${context.teamIndex + 1} / oportunidad ${context.attemptIndex + 1}</em>
      </div>
      <div class="cp-bottom-actions">
        ...
        <button class="button primary cp-save-score-button" data-action="next-score">${renderCpIcon("check")} Guardar y siguiente</button>
      </div>
    </footer>
  `;
}
...
function deleteCharreada(charreadaId) {
  ...
  if (state.activeCharreadaId === charreadaId) {
    ...
    state.activeCharreadaId = nextCharreada?.id || null;
    resetScoringPointer();
  }
...
function getScoreNodeForContext(context) {
  if (!context?.tournament?.id || !context?.charreada?.id || !context?.team?.id || !context?.suerte?.id) return null;
  const id = scoreKey(context.charreada.id, context.team.id, context.suerte.id);
  return {
    id,
    tournamentId: context.tournament.id,
    payload: JSON.parse(JSON.stringify(state.scores[id] || []))
  };
}
...
async function publishCurrentScoreNode(context) {
  ...
  if (!scoreNode) {
    setLastFirebaseError("missing-score", "No se pudo resolver la calificacion actual.");
    showToast("No se pudo guardar la calificacion en CharroPro.");
    return { ok: false, reason: "missing-score" };
  }

  const result = await publishFirebaseScore(scoreNode.tournamentId, scoreNode.id, scoreNode.payload, getAccessActor());
  if (!result.ok) {
    setLastFirebaseError(result.reason || "score-save-failed", result.detail?.message || "");
    showToast("No se pudo guardar la calificacion en CharroPro.");
  }
  return result;
}
...
async function nextScore() {
  ...
  if (context) {
    ...
    saveState({ silent: true });
    const scoreResult = await publishCurrentScoreNode(context);
    if (!scoreResult.ok) return;
    const published = recordPublishedScore(buildPublishedScoreSnapshot(context));
    if (published) void publishFirebaseAuditScore(published);
  }

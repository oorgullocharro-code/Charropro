const LIFECYCLE = Object.freeze({ PRE_EVENT: "PRE_EVENT", LIVE: "LIVE", PAUSED: "PAUSED", FINALIZED: "FINALIZED", ARCHIVED: "ARCHIVED" });
...
export function resolveCanonicalTournamentLifecycle({ tournament = {}, liveCurrent = {} } = {}) {
  ...
  const explicitStatuses = [liveCurrent.lifecycleStatus, liveCurrent.status, info.lifecycleStatus, info.status, info.estado, tournament.lifecycleStatus, tournament.status].map(normalizeStatus);
  ...
  const activeCharreadaId = firstId(liveCurrent.activeCharreadaId, liveCurrent.charreadaId, liveCurrent.charreada?.id, liveCurrent.turn?.charreadaId);
  if (activeCharreadaId && charreadas.some((charreada) => id(charreada.id || charreada.charreadaId) === activeCharreadaId)) return resolution(LIFECYCLE.LIVE, "live-current-active-charreada");
  if (charreadas.some((charreada) => LIVE.has(normalizeStatus(charreada.status || charreada.estado)))) return resolution(LIFECYCLE.LIVE, "program-active-charreada");
  return resolution(LIFECYCLE.PRE_EVENT, "no-authoritative-live-state");
}

export const CANONICAL_TOURNAMENT_LIFECYCLE = LIFECYCLE;
function resolution(status, source) { return Object.freeze({ status, source }); }
...
function id(value) { const clean = String(value || "").trim(); return /^[A-Za-z0-9._:@/-]{1,180}$/.test(clean) ? clean : ""; }
function firstId(...values) { for (const value of values) { const resolved = id(value); if (resolved) return resolved; } return ""; }

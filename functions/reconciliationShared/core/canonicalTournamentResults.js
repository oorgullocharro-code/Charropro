...
export function buildCanonicalTournamentResults(source = {}, options = {}) {
  ...
  for (const record of collection(canonical.currentRecords)) {
    ...
    if (!rows.has(key)) rows.set(key, { resultId: stableId("result", [identity.competitionId, identity.phaseId, identity.charreadaId, identity.participantScope, identity.entityId]), tournamentId, competitionId: identity.competitionId || "competition", competitionName: charreadaName(charreadas, identity.charreadaId), phaseId: identity.phaseId || "", phaseName: phaseName(charreadas, identity.charreadaId), charreadaId: identity.charreadaId, participantScope: identity.participantScope, teamId: identity.teamId, teamName: identity.teamName || teamName(teams, identity.teamId), participantId: identity.participantId, participantName: identity.participantName, suertes: {}, recordIds: [], sourceRevision: 0 });
  ...
  for (const row of items) {
    if (!byCompetition.has(row.competitionId)) byCompetition.set(row.competitionId, { competitionId: row.competitionId, name: row.competitionName, rows: [] });
    byCompetition.get(row.competitionId).rows.push({ resultId: row.resultId, teamId: row.teamId, teamName: row.teamName, participantId: row.participantId, participantName: row.participantName, total: row.total, columns: Object.fromEntries(Object.entries(row.suertes).map(([key, value]) => [key, value.total])) });
  }

...
import { formatOfficialTimerMs } from "../js/core/officialTimerLiveDisplay.js?v=20260830-negative-timing-attempt-v2-official-publication-recovery-001-v1";
...
assert.match(card, /width:\s*min\(320px, calc\(100vw - 24px\)\)/);
assert.match(card, /container-type:\s*inline-size/);
assert.match(card, /box-sizing:\s*border-box/);
assert.match(value, /font-size:\s*clamp\(28px, 15cqw, 46px\)/);

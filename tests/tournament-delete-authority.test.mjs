...
const preflight = buildTournamentDeletionPreflight(cleanSource, tournamentId);
...
const officialPreflight = buildTournamentDeletionPreflight(officialSource, tournamentId);
assert.equal(officialPreflight.hasOfficialScores, true);
assert.deepEqual(officialPreflight.blockingReasons, ["tournament-has-official-history"]);
const auditSource = source();
auditSource.audit.publishedScores = { audit_a: { id: "audit_a" } };
assert.deepEqual(buildTournamentDeletionPreflight(auditSource, tournamentId).blockingReasons, ["tournament-has-official-history"]);

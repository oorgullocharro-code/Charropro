...
import { fixture, tid, input as template, uid, cid, teamId } from './fixtures/historicalReconciliationFixture.mjs';
import { signature, databaseSignature } from '../functions/historicalReconciliation.mjs';
import { getCanonicalOfficialTeamTotals } from '../js/core/canonicalOfficialResults.js';
...
test('Auth + Storage + RTDB + callable: backup/verify/execute/reload/reproject/idempotency/isolation', {skip: process.env.CHARROPRO_RUN_RECONCILIATION_EMULATOR !== '1'}, async()=>{
  ...
  const root=fixture(); root.users[auth.localId]=root.users[uid]; delete root.users[uid];
  ...
  assert.equal(pial.filter(r=>!r.superseded).length,1);assert.equal(pial.filter(r=>r.superseded).length,2);
  ...
  console.log('EMULATOR_CERTIFICATION=PASS CURRENT_HEADS=1 HISTORICAL=2 CANONICAL_PR=21 INTERNAL_PR=21 PUBLIC_PR=21 TOTAL=193');
});
